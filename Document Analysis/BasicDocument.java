...
public class BasicDocument extends Document 
{
	...
	public int getNumSyllables()
	{
		...
		String text_input = getText();
		String out_word = "";
		int number_of_syllables = 0;
		
		//loop to scan throw each word.
		String[] hold_words = text_input.split("\\s+");
		for (int i = 0; i < hold_words.length; i++)
		{
		    out_word = hold_words[i].replaceAll("[^a-zA-Z]", ""); //"[^\\w]""
		    int hold_num = countSyllables(out_word);
		    number_of_syllables += hold_num;
		}
